...
def main():
    ...
    if 'Loan_Status' not in train_p.columns:
        raise KeyError("Loan_Status missing in train_cleaned.csv")
    ...
    print(f"Naive Bayes training accuracy: {acc*100:.2f}%")
    out_train = train.copy()
    out_train['Loan_Status_Predicted'] = ['Y' if v==1 else 'N' for v in train_pred]
    out_train.to_csv("trained_naive_bayes.csv", index=False)
    print("Saved trained_naive_bayes.csv")
    if test_p is not None:
        ...
        out_test.to_csv("test_naive_bayes.csv", index=False)
        print("Saved test_naive_bayes.csv")
    else:
